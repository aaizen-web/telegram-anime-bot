...
def init_pool():
    ...
    connection_pool = pool.SimpleConnectionPool(2, 10, dsn=DATABASE_URL, sslmode='require')
...
def build_episode_keyboard(episodes, season_id, anime_id, page=0):
    ...
    if len(columns) > 1:
        ...
        for col_idx in range(len(columns)):
            ...
            header_row.append(InlineKeyboardButton("Ep " + str(col_start) + "-" + str(col_end), callback_data="noop"))
    ...
    for row in range(max_rows):
        ...
        for col in columns:
            if row < len(col):
                ep = col[row]
                ep_row.append(InlineKeyboardButton("Ep " + str(ep[0]), callback_data="episode|" + str(anime_id) + "|" + str(season_id) + "|" + str(ep[0])))
            else:
                ep_row.append(InlineKeyboardButton(" ", callback_data="noop"))
        if ep_row:
    ...
    if page > 0:
        nav.append(InlineKeyboardButton("⬅ Prev", callback_data="eppage|" + str(anime_id) + "|" + str(season_id) + "|" + str(page-1)))
    if end < total_episodes:
        nav.append(InlineKeyboardButton("Next ➡", callback_data="eppage|" + str(anime_id) + "|" + str(season_id) + "|" + str(page+1)))
    if nav:
        keyboard.append(nav)
    keyboard.append([InlineKeyboardButton("🔙 Back to Seasons", callback_data="seasons|" + str(anime_id))])
    return keyboard
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("""INSERT INTO users (user_id, last_active, total_requests) VALUES (%s, CURRENT_TIMESTAMP, 1)
        ON CONFLICT (user_id) DO UPDATE SET last_active = CURRENT_TIMESTAMP, total_requests = users.total_requests + 1""", (user_id,))
    conn.commit()
    cursor.close()
    release_conn(conn)
...
async def check_join(update, context):
    ...
    if not await is_user_member(context.bot, user_id):
        ...
        await query.edit_message_text("❌ You have not joined the channel yet!\n\nPlease join and then click 'I Joined!' again:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("""INSERT INTO users (user_id, last_active, total_requests) VALUES (%s, CURRENT_TIMESTAMP, 1)
    ...
    if user_id == ADMIN_ID:
        keyboard.append([InlineKeyboardButton("🛠 Admin Panel", callback_data="admin_panel")])
    await query.edit_message_text("✅ Thank you for joining! Welcome!\n\nChoose an option:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_anime(update, context):
    ...
    if (page + 1) * ANIME_PER_PAGE < total:
        nav_buttons.append(InlineKeyboardButton("Next ➡", callback_data="page|" + str(page+1)))
    if nav_buttons:
...
async def show_seasons(update, context):
    ...
    cursor.execute("SELECT id, season_number FROM seasons WHERE anime_id = %s ORDER BY season_number ASC", (anime_id,))
    seasons = cursor.fetchall()
    cursor.close()
    release_conn(conn)
    if not seasons:
        await query.edit_message_text("❌ No seasons found for " + anime_name + ".", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="show_anime")]]))
        return
    keyboard = []
    for season in seasons:
        season_id, season_number = season
        keyboard.append([InlineKeyboardButton("🎬 Season " + str(season_number), callback_data="season_ep|" + str(anime_id) + "|" + str(season_id) + "|0")])
    keyboard.append([InlineKeyboardButton("🔙 Back", callback_data="show_anime")])
    await query.edit_message_text("📺 " + anime_name + "\n\nSelect a season:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_episodes(update, context):
    ...
    cursor.execute("SELECT episode_number FROM episodes WHERE season_id = %s ORDER BY episode_number ASC", (season_id,))
    episodes = cursor.fetchall()
    cursor.close()
    release_conn(conn)
    if not episodes:
        await query.edit_message_text("❌ No episodes found for Season " + str(season_number) + ".", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="seasons|" + str(anime_id))]]))
        return
    ...
    await query.edit_message_text("📺 " + anime_name + " — Season " + str(season_number) + "\n🎬 Episodes " + str(start) + "-" + str(end) + " of " + str(total) + ":", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def send_episode(update, context):
    ...
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT file_id FROM episodes WHERE season_id = %s AND episode_number = %s", (season_id, episode_number))
    ...
    cursor.execute("SELECT season_number FROM seasons WHERE id = %s", (season_id,))
    season_number = cursor.fetchone()[0]
    video_msg = await query.message.reply_video(video=file_id, caption="Season " + str(season_number) + " — Episode " + str(episode_number))
    cursor.execute("INSERT INTO watch_history (user_id, anime_id, season_id, episode_number) VALUES (%s, %s, %s, %s)", (query.from_user.id, anime_id, season_id, episode_number))
    conn.commit()
    cursor.close()
    release_conn(conn)
...
async def handle_channel_video(update, context):
    ...
    if update.channel_post and update.channel_post.video:
        ...
        cursor.close()
        release_conn(conn)
        print("✅ " + anime_name + " | Season " + str(season_number) + " | Episode " + str(episode_number) + " saved.")
...
async def add_anime(update, context):
    ...
    cursor.close()
    release_conn(conn)
    await update.message.reply_text("✅ " + anime_name + " added successfully.")
...
async def add_season(update, context):
    ...
    cursor.close()
    release_conn(conn)
    await update.message.reply_text("✅ Season " + str(season_number) + " added to " + anime_name + ".")

async def bulk_add(update, context):
    if update.effective_user.id != ADMIN_ID:
        return
    if not context.args:
        await update.message.reply_text("Usage: /bulk_add Anime Name | Season Number | StartEp-EndEp\nExample: /bulk_add Naruto | 1 | 1-24")
        return
    text = " ".join(context.args)
    parts = text.split("|")
    ...
    anime_name = parts[0].strip()
    season_number = int(parts[1].strip())
    ep_range = parts[2].strip().split("-")
    start_ep = int(ep_range[0].strip())
    end_ep = int(ep_range[1].strip())
    if start_ep > end_ep:
    ...
    result = cursor.fetchone()
    if not result:
        await update.message.reply_text("Anime not found. Add it first with /add_anime")
        cursor.close()
        release_conn(conn)
        return
    anime_id = result[0]
    ...
    lines = [anime_name + " | " + str(season_number) + " | " + str(ep) for ep in range(start_ep, end_ep + 1)]
    caption_text = "\n".join(lines)
    await update.message.reply_text("✅ Season " + str(season_number) + " ready for " + anime_name + "!\n\nNow upload " + str(total) + " videos to your storage channel with these captions in order:\n\n" + caption_text)
...
async def add_episode(update, context):
    ...
    if not result:
        await update.message.reply_text("Anime not found.")
        return
    await update.message.reply_text("Now upload the episode video to the storage channel with caption:\n\n" + anime_name + " | " + str(season_number) + " | " + str(episode_number))
...
async def delete_episode(update, context):
    ...
    cursor.close()
    release_conn(conn)
    await update.message.reply_text("✅ Episode " + str(episode_number) + " of Season " + str(season_number) + " deleted.")
...
async def delete_season(update, context):
    ...
    cursor.close()
    release_conn(conn)
    await update.message.reply_text("✅ Season " + str(season_number) + " and all its episodes deleted.")
...
async def delete_anime(update, context):
    ...
    cursor.close()
    release_conn(conn)
    await update.message.reply_text("✅ " + anime_name + " and all its seasons/episodes deleted.")
...
async def handle_admin_actions(update, context):
    ...
    if action == "admin_add_anime":
        ...
        await query.message.reply_text("Send: Anime Name | Season Number | StartEp-EndEp\nExample: Naruto | 1 | 1-24")
...
async def handle_admin_text(update, context):
    ...
    if action == "admin_add_anime":
        ...
        conn.commit()
        await update.message.reply_text("✅ " + text + " added successfully.")
        ...
        season_number = int(parts[1].strip())
        episode_number = int(parts[2].strip())
        await update.message.reply_text("Now upload video in channel with caption:\n" + anime_name + " | " + str(season_number) + " | " + str(episode_number))

    elif action == "admin_bulk_add":
        parts = text.split("|")
        if len(parts) != 3 or "-" not in parts[2]:
            await update.message.reply_text("Format: Anime Name | Season Number | StartEp-EndEp\nExample: Naruto | 1 | 1-24")
            cursor.close()
            release_conn(conn)
            return
        anime_name = parts[0].strip()
        ...
        lines = [anime_name + " | " + str(season_number) + " | " + str(ep) for ep in range(start_ep, end_ep + 1)]
        caption_text = "\n".join(lines)
        await update.message.reply_text("✅ Ready! Now upload " + str(total) + " videos with these captions:\n\n" + caption_text)
...
async def show_analytics(update, context):
    ...
    cursor.execute("SELECT animes.name, COUNT(*) as views FROM watch_history JOIN animes ON watch_history.anime_id = animes.id GROUP BY animes.id, animes.name ORDER BY views DESC LIMIT 1")
    result = cursor.fetchone()
    ...
    await query.edit_message_text(
        "📊 Analytics\n\n👥 Total Users: " + str(total_users) + "\n🎬 Total Episode Views: " + str(total_views) + "\n🔥 Most Watched Anime: " + top_anime + " (" + str(top_views) + " views)",
